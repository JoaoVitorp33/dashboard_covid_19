...
headers = {
	"X-RapidAPI-Key": "12a1748589msh3ff571189e44214p113558jsn1d1779d3f986",
...
df = pd.DataFrame(lista_info, columns=['continente', 'pais', 'casos', 'recuperados', 'casos ativo', 'mortes', 'dia'])

# organizando por continentes e geral
df_total = df.groupby(['continente']).sum()
...
lista_continentes_valor = [Total[0]['casos'], Total[2]['casos'], Total[3]['casos'], Total[4]['casos'], Total[5]['casos'], Total[6]['casos']]
lista_continentes_nome = [Total[0]['nome'], Total[2]['nome'], Total[3]['nome'], Total[4]['nome'], Total[5]['nome'], Total[6]['nome']]
...
# classificação de paises mais afetados
...
for i in df_pais_top.head(11).values.tolist():
    if i[0] == "All" or i[0] == "Asia" or i[0] == "North-America" or i[0] == "Europe" or i[0] == "South-America" or i[0] == "Africa":
        pass
    else:
        temp = []
        temp.append(i[0])
